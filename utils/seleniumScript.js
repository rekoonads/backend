      ...
      if (!found) {
        ...
        try {
          ...
          const rows = await driver.findElements(
            By.css("tbody tr:has(td a.inlineIcon.iconCampaign)")
          );

          for (let row of rows) {
            // const campaignElement = await row.findElement(By.css('td a.inlineIcon.iconCampaign'));
            const campaignElement = await driver.wait(
              until.elementLocated(By.css("td a.inlineIcon.iconCampaign")),
              10000
            );
            ...
            if (campaignText === campaignName) {
              ...
              await driver.wait(
                until.elementLocated(By.css(".tableWrapper > tbody > tr")),
                5000
              );
